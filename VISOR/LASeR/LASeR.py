...
def run(parser, args):
    ...
    logging.basicConfig(
        filename=os.path.abspath(args.output + "/VISOR_LASeR.log"),
        filemode="w",
        level=logging.DEBUG,
    ...
    if not os.access(os.path.dirname(os.path.abspath(args.genome)), os.W_OK):
        ...
        logging.error(
            "It is required to have write access on reference folder to generate minimap2 .mmi indexes"
        )
            ...
            try:
                ...
                subprocess.check_call(
                    [
                        "minimap2",
                        "-d",
                        os.path.abspath(args.genome + ".ont.mmi"),
                        os.path.abspath(args.genome),
                    ],
                    stderr=open(os.devnull, "wb"),
                    stdout=open(os.devnull, "wb"),
                )  # create .mmi: faster when it comes to simulate multiple times from same chromosome
            ...
            try:
                ...
                subprocess.check_call(
                    [
                        ...
                        "-H",
                        "-d",
                        os.path.abspath(args.genome + ".pb.mmi"),
                        os.path.abspath(args.genome),
                    ],
                    stderr=open(os.devnull, "wb"),
                    stdout=open(os.devnull, "wb"),
        ...
        if args.clonefraction is None:
            ...
            for fraction in fractions:
                ...
                try:
                    ...
                    logging.error(
                        "Each fraction percentage in --clonefraction must be float"
                    )
                    exitonerror(os.path.abspath(args.output))
    ...
    if len(inputs) == 1:  # just one folder, use a classic simulation
        ...
        fastas = sorted(
            glob.glob(os.path.abspath(inputs[0] + "/*.fa")), key=natural_keys
        )
        ...
        for folder, fasta in enumerate(fastas):
            ...
            for entries in srtbed:  # validate each entry
                ...
                if str(entries[0]) not in classic_chrs:
                    ...
                    logging.warning(
                        str(entries[0])
                        + " is not a valid chromosome in .bed file.Skipped"
                    )
        ...
        for fract, inp in enumerate(inputs):  # each input is now a subclone
            ...
            subfastas = sorted(
                glob.glob(os.path.abspath(inp) + "/*.fa"), key=natural_keys
            )
...
def exitonerror(output):
    ...
    print(
        ...
        + os.path.abspath(output + "/VISOR_LASeR.log")
        + " for more details."
    )
    sys.exit(1)
    ...
    if not allelic == 100:  # simulate part from modified and part from reference
        ...
        with open(os.path.abspath(output + "/sim_0001.fastq"), "w") as regionout:
            ...
            subprocess.call(
                [
                    "cat",
                    os.path.abspath(output + "/simref_0001.fastq"),
                    os.path.abspath(output + "/simvar_0001.fastq"),
                ],
                stdout=regionout,
                stderr=open(os.devnull, "wb"),
            )
        ...
        with open(os.path.abspath(output + "/region.tmp.sam"), "w") as samout:
            ...
            subprocess.call(
                [
                    ...
                    "--cs",
                    new_mmi,
                    os.path.abspath(output + "/sim_0001.fastq"),
                ],
                stdout=samout,
                stderr=open(os.devnull, "wb"),
            )
