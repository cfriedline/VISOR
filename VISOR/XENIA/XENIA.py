...
def run(parser, args):
    ...
    barcodepath = os.path.abspath(
        os.path.dirname(__file__) + "/4M-with-alts-february-2016.txt.gz"
    )
    ...
    for folder, fasta in enumerate(fastas):
        ...
        for entries in srtbed:  # validate each entry
            ...
            if str(entries[0]) not in allchrs:
                ...
                print(
                    str(entries[0]) + " is not a valid chromosome in .bed file. Skipped"
                )
                continue
    ...
    subprocess.call(
        ["bash", cleanerpath, os.path.abspath(args.output)]
    )  # refine FASTQ so that they are readily usable with longranger
...
def randomlong(Par, refseq, N_frag):
    ...
    for i in range(N_frag):
        ...
        length = int(
            np.random.exponential(scale=Par.LMOL)
        )  # molecules length is randomly distributed according to exponential distribution
...
def Runner(processor, molecule, refseq, reftitle, refstart, Par, output):
    ...
    for mol in molecule:
        ...
        with open(
            os.path.abspath(output + "/" + processor + "." + moleculenumber + ".fa"),
            "w",
        ) as fout:
        ...
        with open(
            os.path.abspath(
                os.path.dirname(output) + "/" + processor + ".processed_molecules"
            ),
            "a",
        ) as statout:
        ...
        subprocess.call(
            [
                ...
                str(Par.INDELEXT),
                os.path.abspath(
                    output + "/" + processor + "." + moleculenumber + ".fa"
                ),
                os.path.abspath(
                    output + "/" + processor + "." + moleculenumber + ".R1.fq.tmp"
                ),
                os.path.abspath(
                    output + "/" + processor + "." + moleculenumber + ".R2.fq"
                ),
            ],
            stderr=open(os.devnull, "wb"),
        ...
        with open(
            ...
            os.path.abspath(output + "/" + processor + "." + moleculenumber + ".R1.fq"),
            "w",
        ) as fout:
...
def LinkedSim(Par, reftitle, refseq, refstart, output):
    ...
    with open(
        os.path.abspath(os.path.dirname(output) + "/assigned_barcodes.txt"), "a"
    ) as barcodesout:
    ...
    for i, molecule in enumerate(slices):
        ...
        p = multiprocessing.Process(
            target=Runner,
            args=(processor, molecule, refseq, reftitle, refstart, Par, output),
        )
