...
def run(parser, args):
    ...
    try:
        ...
        srtbed = (
            bed.sort()
        )  # sorting here is not necessary, but allows to look for general formatting errors
    ...
    if len(inputs) > 1:
        ...
        if args.clonefraction is None:
            ...
            logging.error(
                "When specifying multiple -s/--sample, multiple --clonefraction percentages must be specified"
            )
            exitonerror(os.path.abspath(args.output))
            ...
            if len(fractions) != len(inputs):
                ...
                logging.error(
                    "When specifying multiple -s/--sample, the same number of --clonefraction percentages must be specified"
                )
                exitonerror(os.path.abspath(args.output))
            ...
            if totalfraction > 100:
                ...
                logging.error(
                    "Sum of fractions percentages in --clonefraction cannot exceed 100.0"
                )
                exitonerror(os.path.abspath(args.output))
            ...
            for entries in srtbed:  # validate each entry
                ...
                try:
                    ...
                    logging.error(
                        "Cannot convert "
                        + str(entries[4])
                        + " to float number in .bed file. Sample fraction must be a float percentage"
                    )
                    exitonerror(os.path.abspath(args.output))
